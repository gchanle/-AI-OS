...
export default function TasksModal({ isOpen, onClose, tasks, onSelectSession, onDeleteTask, onSaveEdit, editingTaskId, setEditingTaskId, editingTitle, setEditingTitle }) {
    ...
    const filteredTasks = tasks.filter(t => {
        ...
        if (activeTab === 'stopped') return t.status === 'stopped';
        return true;
    });
                                    ...
                                    <div className="task-header-modal">
                                        {editingTaskId === t.id ? (
                                            <input 
                                                ...
                                                onKeyDown={e => { if (e.key === 'Enter') onSaveEdit(t.id); }}
                                                placeholder="按回车保存"
                                        ...
                                        <div style={{ display: 'flex', gap: '4px' }}>
                                            ...
                                            <button className="task-delete-btn" onClick={(e) => { e.stopPropagation(); onDeleteTask(t.id); }} title="移出任务">
