...
function sortEntries(entries = []) {
    return [...entries].sort((left, right) => {
        const leftPinned = left.crossThreadPinned ? 1 : 0;
        const rightPinned = right.crossThreadPinned ? 1 : 0;
        if (rightPinned !== leftPinned) {
            return rightPinned - leftPinned;
        }

        const leftFrozen = left.frozen ? 1 : 0;
        const rightFrozen = right.frozen ? 1 : 0;
        if (rightFrozen !== leftFrozen) {
            return rightFrozen - leftFrozen;
        }
...
function computeMemoryScore(entry, {
    ...
    if (threadKey && entry.threadKey === threadKey) {
        score += 3;
    } else if (threadKey && entry.crossThreadPinned) {
        score += 3;
    }
        ...
        ...groupItems.map((group) => [
            ...
            ...group.items.map((entry, index) => (
                [
                    ...
                    entry.note ? `- 备注：${truncate(entry.note, 120)}` : '',
                ].filter(Boolean).join('\n')
            )),
        ].join('\n\n')),
    ].join('\n\n');
...
function buildTaskMemoryEntry(task = {}, options = {}) {
    ...
    const defaultPriorityBand = ['working', 'standard', 'high', 'critical'].includes(String(options.defaultPriorityBand || '').trim())
        ? String(options.defaultPriorityBand).trim()
        : '';
    ...
    return {
        ...
        priorityBand: status === 'failed'
            ? 'high'
            : defaultPriorityBand || (status === 'completed' ? 'standard' : 'working'),
...
export async function getFireflyMemoryEntryById(memoryId = '') {
    ...
    return entries.find((item) => item.id === normalizedId) || null;
}

export async function queryFireflyMemoryEntries({
    uid = '',
    fid = '',
    threadKey = '',
    question = '',
    ...
    const typeFilter = normalizeArray(memoryTypes);
    const scored = (await listFireflyMemoryEntries())
        .filter((entry) => {
            if (uid && entry.uid && entry.uid !== uid) {
                return false;
            }
            if (fid && entry.fid && entry.fid !== fid) {
                return false;
            }
            if (threadKey && entry.threadKey && entry.threadKey !== threadKey && !entry.crossThreadPinned) {
                return false;
            }
            if (typeFilter.length > 0 && !typeFilter.includes(entry.memoryType)) {
                return false;
            }
            return true;
        })
    ...
    if (scored.length > 0) {
        await touchFireflyMemoryEntries(scored.map((item) => item.id));
    }
...
export async function removeFireflyMemoryEntries(memoryIds = []) {
    ...
    return enqueueMemoryMutation(async (state) => {
        const removed = (state.entries || []).filter((entry) => ids.includes(entry.id));
...
export async function summarizeFireflyMemoryEntries({
    uid = '',
    fid = '',
    threadKey = '',
    title = '',
    ...
    const entries = await queryFireflyMemoryEntries({
        uid,
        fid,
        threadKey,
        question,
    ...
    const summary = truncate([
        entries.length ? `聚合 ${entries.length} 条记忆` : '',
        entries.slice(0, 4).map((item) => item.summary || item.title).filter(Boolean).join('；'),
    ].filter(Boolean).join('｜'), 220);
    ...
    const compressedEntry = {
        id: `memory-rollup:${threadKey || uid || 'shared'}:${Date.now()}`,
        uid,
        fid,
        threadKey,
        title: String(title || '记忆压缩摘要').trim(),
        summary,
        detail,
        capabilityIds: normalizeArray(capabilityIds),
        tags: ['agent_memory_summary'],
        source: 'firefly_agent_memory',
        memoryLayer: 'compressed',
        memoryType: 'workflow_hint',
        anchorType: threadKey ? 'thread' : 'user',
        visibility: 'runtime',
        retentionPolicy: 'compressed_rollup',
        priorityBand: 'high',
        sourceTaskIds: entries.map((item) => item.taskId).filter(Boolean),
        importance: 5,
        note: '',
        frozen: false,
        crossThreadPinned: false,
    ...
    return upsertFireflyMemoryEntry(compressedEntry);
}
